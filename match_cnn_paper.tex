...
for this study. The input datasets consist of simulated gravitational-wave
timeseries data that has been whitened in the frequency domain. This process
rescales the noise contribution at each frequency to have equal power resulting
in ``white" noise. Signal+noise data has its constituent frequency components
...
uniform prior on the range $[0,2\pi]$, and the inclination angle is drawn such
...
such that the peak amplitude of each waveform is randomly positioned within the
fractional range $[0.75,0.95]$ of the 1\,sec timeseries. 
...
predefined optimal \ac{SNR} of values $ \rho_{\mathrm{opt}} = 2, 4, 6, 8, 10, 12 $.
...
with the gray-scale intensity of each pixel proportional to the
...
updating the weight and bias terms accordingly.  Back propagation is done over
multiple iterations called \textit{epochs}. 
...
The ranking statistic that we extract from the \ac{CNN} analysis is taken from
the final output layer, composed of 2 neurons, where each neuron will output a
probability value between zero and 1. One neuron gives the probability that the
input data does not contains a signal and the second gives the probability that
the input contains a signal.
...
\textit{Results.}---
%
After tuning the multiple hyper-parameters (Table
\ref{table:network}) and training the neural network, we present the results of
our classifier on a noise versus signal+noise sample set. Our training
uses a transfer-learning approach whereby the network is initially trained on
a sample set of 1000 noise and 1000 injection signals (each with 25 different
noise realizations) with \ac{SNR}$=12$. The \ac{SNR} value is then decreased by
1 and using the same trained weights from our previous network we trained our
classifier again. This is repeated until the network trains on data of a
desired value of \ac{SNR}. We find that the performance benefit of
transfer-learning is marginal.
...
approach broadly matches the sensitivity of matched-filtering for all test
...
approches and ...\chris{We need to look at the final results before we write
...
analyses were performed.\label{fig:efficiency_curve}} 
\end{figure}
...
convolutional neural network with carefully chosen hyperparameters and produce
...
existing matched-filtering approaches.
...
LIGO-Virgo Collaboration specifically from T.~Dent, M.~Cavalgia, and the
machine-learning working group. The authors also gratefully acknowledge the
