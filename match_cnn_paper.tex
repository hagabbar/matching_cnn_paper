...
reproduce the sensitivity of a matched-filter search for binary black hole gravitational wave signals. The standard method for the detection of well
modelled transient gravitational wave signals is matched filtering. However,
...
wave time-series~\fergus{Toms suggestion: a whitened time
series of measured GW strain} as an input and train and test on simulated binary black hole
signals in Advanced detector Gaussian noise~\fergus{Toms suggestion: synthetic
Gaussian noise representative of [final] Advanced LIGO [design]
sensitivity}. We show that our network can classify signal from non-signal with a performance
...
to be comparable in sensitivity as defined by Reciever-Operator
characteristics. 
%
\end{abstract}
...
multiple post-merger electromagnetic signatures~\cite{2017arXiv171005833L}~\fergus{Tom comments: Not sure why you are citing the Fermi and Integral papers and the MMA 
paper here - which are not relevant to detection of CBC signals in GW 
data - when you don't include citations to the basic CBC matched filter 
literature or to recent work on template banks}.
...
%
The algorithms used to make these detections \cite{0264-9381-33-21-215004, 0004-637X-748-2-136, PhysRevD.90.082004} are computationally expensive to run.
...
space using advanced signal processing techniques~\fergus{Tom suggests remove this}. Distinguishing noise from
...
PhysRevD.53.6749, PhysRevD.60.022002, 0264-9381-23-18-002, PhysRevD.80.104014, Blanchet2014, PhysRevD.89.061502, PhysRevD.46.5236, PhysRevD.47.2198, PhysRevD.85.122006}. 
~\fergus{Tom comments: its hard to give a proper set of citations 
unless you separate the idea of the bank from the idea of the waveforms.
Assuming that we're talking about the bank construction, your citations 
run out in 2009.  However the banks used in O1 and O2 use more recent 
methods.  Should cite \cite{1705.01845,PhysRevD.90.082004,PhysRevD.89.024003,PhysRevD.89.084041,PhysRevD.86.084017}. 
Optionally remove the old refs as obsolete (they consider 
inaccurate or incomplete waveform models).}
A template bank will span a large astrophysical parameter space since we do not
...
theory~\cite{PhysRevD.84.049901,PhysRevD.80.084043,Blanchet2014,PhysRevD.93.084054}~\fergus{Tom comments:
it's not true that short BBH waveforms are well modelled by PN 
theory. You need merger and ringdown as well and fitting of the complete waveform to NR},
...
routines called \texttt{LALSuite}~\fergus{maybe prefer LALSimulation as that is the home of waveforms. 
Please cite it via URL}. We use the IMRPhenomD type
...
at 8192 Hz.~\fergus{Tom suggests: at this point you haven't said how long the time series is. 
Better mention it earlier, as well as the chosen values of SNR} Therefore we consider $f_{\mathrm{min}}$ as the
...
use in a matched filtering approach~\fergus{Tom suggests: Should cite where you got the formulae 
from (Owen-Sathya?).  You should also mention that the correct density 
of templates for the actual inspiral-merger-ringdown waveforms is 
expected to be (somewhat) different. Tom suggests citing \cite{PhysRevD.91.124042}}. It is logical to place waveforms closer in
...
signal (cyan)~\fergus{Marco asks: Is the cyan curve really
    whitened?} with component masses $m_{1}=41.86\mathrm{M}_{\odot}$ and
...
used in this case is the matched-filter \ac{SNR} analytically maximised over arrival time,
phase and distance.~\fergus{Tom comments: matched filter 
automatically does the analytical maximization over distance and phase, but NOT over time. This is done numerically.} 
By first defining the noise weighted inner
...
reference}~\fergus{Tom also suggests naming the specific tools, and the value of flow used}. 
The template bank in this case contains $?$ individual templates. 
...
%
\chris{remove this paragraph?} After tuning several hyperparameters and then settling on
...
$\optsnr=12$.
%\chris{maybe plot 2x3 rather than 3x2 if we have the space. Also
%we have to anser the question of why the SNR=2 case mostly predicts an signal,
...
filtering method between SNR 2 and SNR 4.\chris{I rewrote this paragrpah
...
\caption{~\fergus{Marco comments: different curves in some of the plots are 
really difficult to distinguish}The \ac{ROC} curves for test datasets containing 
...
%\chris{OK, first can we make these plots 2x3 rather
...
%log -scale. That should be enough to get on with.}
...
LIGO O1 or O2 einstein at home paper}. Whereas~\fergus{Marcos says: Whereas... and following seems
incomplete} unsupervised approaches have the
potential to be powerful detection tools in searches for unmodelled burst-like
gravitational-wave signals.~\fergus{Maybe we could also mention the potential for
 parameter estimation here also?} 
