...
series of measured gravitational-wave strain as an input, and we train and test on simulated binary black hole
signals in synthetic Gaussian noise representative of Advanced LIGO sensitivity. 
We show that our network can classify signal from non-signal with a performance\fergus{in the case
of if having to change the letter to account for new results, I have highlighted red all references to the
CNN/matched filter performance outside the Results section to make sure we do not miss anything.} {\color{red}comparable to that of match filtering} applied to the same datasets when considering the sensitivity 
...
neurons, which can be anywhere from one to several layers deep. A neuron acts as a filter, 
whereby it performs a transformation on an array of inputs. This transformation is a linear operation 
between the input array and the weight and bias parameters assosicated to the neuron. The resulting 
array is then typically passed to a non-linear activation function to constrain the neuron output 
to be within a set range. Deep learning algorithms typically consist of an input layer, followed by 
one to several hidden layers and then one to multiple output neurons. The scalars produced from the 
output neurons can be used to solve classification problems, where each output neuron corresponds to 
the probability that a particular input sample is of a certain class.
...
to {\color{red}achieve the same sensitivity as established matched-filtering techniques.} 
...
matched filtering, we distinguish between only two cases, \ac{BBH} merger signals
...
such that the peak of each waveform is then randomly placed within the final $ 20\% $ 
of the time series 

\fergus{Instead ``The simulated time series were chosen to be 2s in 
duration. The waveforms were then placed within the timeseries such that the peak 
of each waveform are in the interval [1.25, 1.45]s, such that the waveform approximately 
lies within the middle 1s. The time series is sampled at 8,192 Hz.'', or something like that ,
replacing the previous sentence and some that follow. We would then have to alter the next 
paragraph to account for the network only receiving the middle 1s, and the timeseries being 
cut with the tukey window, straddling the middle 1s for the matched filter}. 

The waveform amplitude is scaled to achieve a predefined optimnal \ac{SNR} defined as
%
% the optimal SNR
%
...
at 8,192 Hz. Therefore we consider $f_{\mathrm{min}}$ as the
...
%
Supervised deep learning requires datasets to be sub-divided into training, validation, and testing sets. Training sets are the data samples that the network learns from, the validation
...
use 1,000 unique waveforms in the $m_{1},m_{2}$ mass space. Each data sample
timeseries is then represented in the form of a $1 \times 8,192$ pixel image
...
image which, in our case, is a 1-dimensional timeseries vector. As the data is 
1-dimensional, the weight and bias parameters of the network are also in 1-dimensional 
vector form. Each neuron in the convolutional layer computes the convolution between 
the neuron's weight vector and the outputs from the layer below it, and then the 
result is summed with the bias vector. Neuron weight vectors are updated through an 
optimisation algorithm called back-propogation \cite{LeCun1998}. 
...
layer's output unchanged. The activation function can be shifted to better fit network 
predictions by adjusting the bias vector. Pooling layers perform a downsampling 
operation along the spatial dimensions of their input, essentially acting as a data 
reduction process speeding up the training of the network for minimal loss of
information. Finally we have a hidden layer that is connected to a final output layer 
which computes the class scores using an error function, cross-entropy, defined as
%
...
on adaptive moment estimation with incorporated Nesterov momentum (Nadam) \cite{dozat2016incorporating}. The optimization function (back-propagation) works by
...
We used the learning parameters recommended for Nadam in \cite{dozat2016incorporating}, that is a learning rate of $0.002$, $\beta_{1}=0.9$, $beta_{2}=0.999$, $\epsilon = 10^{-8}$ and a momentum schedule of $0.004$. \michael{I know we don't explicitly explain what they are, but we should include there parameters} Further detailed description of the
...
composed of 2 neurons, where each neuron will output a probability value between zero and 1. One 
...
Filter Size  & 32 & 32 & 16 & 16 & 8  & 8  & n/a & n/a & n/a  \\
...
consists of 6 convolutional layers (C), followed by 3 hidden layers (H).
...
. The template bank in this case contains $8,700$ individual templates. 
...
case (orange).
%\chris{So can we change the x-axis ranges to span exactly 2-12
...
%than deep network. Finally change the colours?}
...
gravitational-wave timeseries data, {\color{red}is able to reproduce the results of a
...
capable of {\color{red}reproducing this optimality} we answer a fundamental question
