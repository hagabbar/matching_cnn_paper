...
observed on a more frequent basis~\fergus{Maybe shorten to: ``Over
	the coming years many more such observations are likely to be made on a more frequent basis, alongside signals from more exotic sources such as intermediate black
	holes (IMBH), and neutron star black hole (NSBH) mergers."}. As such, the need for more efficient search
...
comparisons with our model will be described later in this letter~\fergus{You say this at the end of the intro also, so maybe remove this sentence}.
...
medical, bio-science, etc.. }.~\chris{Can you add a sentence or 2
listing the existing machine learning and CNN approaches already published in
GWs, e.g. the George \& Huerta papers and the gravity spy paper at least.} One
...
neurons, which can be anywhere from one to several layers deep~\fergus{`Deepness' is not really explained here. So maybe worth saying ``... composed of stacked arrays ..." or something}. A neuron acts
...
can then either be used to solve classfication, or regression-like problems~\fergus{Regression is the technique applied for both classification and parameter estimation}. In
...
series~\fergus{``The peak of each waveform is then randomly placed within the final $ 20\% $ of the time series"}. The waveform amplitude is scaled to achieve a predefined optimnal
\ac{SNR} defined as
%
% the optimal SNR
%
